# -*- coding: utf-8 -*-
"""
Created on Tue Feb 13 17:28:00 2024
...
from pitch_tracker import find_key_points
...
class VideoProcessor:
    ...
    def process_frame(self, frame):
        ...
        # print(f"Draw objects time: {draw_objects_time - computed_objects_time:.4f} seconds")
    ...
    def send_processed_video_frames(self, host='0.0.0.0', port=8080):
        ...
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        layout_width = self.template_img.shape[1] // 2
        layout_height = self.template_img.shape[0] // 2
        ...
        # read a frame from the center of the video

        ret, frame = cap.read()
        cv2.imwrite(self.config['first_frame'], frame)

        self.key_points, self.key_lines = find_key_points(frame)
        # draw the keypoints and lines on the frame
        frame_with_keypoints = self.key_points.draw(frame)
        frame_with_keypoints_and_lines = self.key_lines.draw(frame_with_keypoints)

        cv2.imwrite(self.config['first_frame_with_keypoints_and_lines'], frame_with_keypoints_and_lines)

        try:
            with tqdm(total=total_frames, desc="Sending processed frames") as pbar:
                while cap.isOpened():
                    ret, frame = cap.read()
                    if not ret:
                        break
                    ...
                    processed_frame, layout_2d, heatmap = self.process_frame(frame)
                    process_frame_time_stamp = time.time()
                    process_frame_time = process_frame_time_stamp - start_time
                    # print(f"Processing time: {process_frame_time:.4f} seconds")
                    ...
                    # Packing the resized frames together
                    start_time = time.time()
                    data = pickle.dumps((small_frame, small_layout, small_heatmap_1, small_heatmap_2))
                    ...
                    print(f"Sending time: {send_time_stamp - start_time:.4f} seconds")
            ...
            if out_composite:
                out_composite.release()
            server_socket.close()
            print('Server shutdown and resources released.')
...
def create_output_dirs(config):
    ...
    config['first_frame'] = os.path.join(output_video_dir, 'first_frame.jpg')
    config['first_frame_with_keypoints_and_lines'] = os.path.join(output_video_dir, 'first_frame_with_keypoints_and_lines.jpg')
...
if __name__ == "__main__":
    # Setup argparse
    parser = argparse.ArgumentParser(description="Process a video for soccer field homography and object detection.")
    parser.add_argument('--input_video_path', type=str, required=True, help='Path to the input video file')
    args = parser.parse_args()

    config = {
        'input_video_path': args.input_video_path,  # Update based on argparse input
